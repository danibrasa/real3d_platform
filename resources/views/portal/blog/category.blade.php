<x-portal-layout>
    ...
    <section class="bg-[#0a0a1e] pt-14 pb-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <span class="inline-block text-white text-xs font-semibold px-4 py-1.5 rounded-full mb-5"
                  style="background: {{ $category->color }}">{{ __('blog.category') }}</span>
            <h1 class="text-4xl md:text-5xl font-extrabold text-white mb-4 tracking-tight">{{ $category->translated_name }}</h1>
            @if($category->translated_description)
                <p class="text-slate-400 max-w-2xl mx-auto text-lg">{{ $category->translated_description }}</p>
            @endif
        </div>
    </section>
                ...
                <div class="lg:col-span-2">
                    ...
                    @else
                        <div class="text-center py-20 bg-white rounded-2xl shadow-sm">
                            ...
                            <p class="text-gray-500 text-lg">{{ __('blog.no_posts_category') }}</p>
                        </div>
                    @endif
                </div>
